	...
	public function product_details($product_id)
	{
		...
		if(!empty($purchaseData)){	
			foreach($purchaseData as $k=>$v){
				$p_date=$purchaseData[$k]['purchase_date'];
				$purchaseData[$k]['final_date'] = $CI->occational->dateConvert(date("Y-m-d",strtotime($p_date)));
		...
		if(!empty($salesData)){	
			foreach($salesData as $k=>$v){
				$salesData[$k]['final_date'] = $CI->occational->dateConvert(date("Y-m-d",strtotime($salesData[$k]['date'])));
